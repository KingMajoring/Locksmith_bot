...
class SQLHandlClientTests(TestCase):
    ...
    def test_get_stock_usage_passes_locksmith_id_as_int_and_since_date(self):
        client = SQLHandlClient()
        fake_conn = _fake_connection([])
        with patch.object(client, "_connection", return_value=fake_conn):
            client.get_stock_usage("42", date(2026, 1, 1))

        cursor = fake_conn.cursor.return_value
        _, params = cursor.execute.call_args[0]
        self.assertEqual(params["locksmith_id"], 42)
        self.assertIsInstance(params["locksmith_id"], int)
    ...
    def test_get_expected_stock_builds_one_placeholder_per_code(self):
        fake_conn = _fake_connection([])
        client = SQLHandlClient()
        with patch.object(client, "_connection", return_value=fake_conn):
            client.get_expected_stock("42", ["TK-100", "TK-101", "TK-102"])
        cursor = fake_conn.cursor.return_value
