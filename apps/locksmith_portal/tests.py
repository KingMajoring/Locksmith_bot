...
class DashboardTests(TestCase):
    ...
    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_dashboard_shows_client_name_phone_and_broker(self, mock_get_optimo, mock_get_handl):
        today = timezone.localdate()
        order_no = f"1001_{today.isoformat()}"
        mock_optimo = MagicMock()
        mock_optimo.list_orders_for_date.return_value = [
        ...
        mock_handl.get_job_details.return_value = {
            "1001": JobDetails(
                report_id="1001", make="Ford", model="Focus", year="2020", reg="AB20 CDE", vin="VIN1",
                service_type="Car", loss_type="LOST", supplied_service="", net_cost=100.0,
                client_name="Sarah Jones", client_phone="07700900123", broker="Admiral",
            )
        }
        mock_get_handl.return_value = mock_handl
        ...
        self.assertContains(response, "Sarah Jones")
        self.assertContains(response, "Admiral")
        self.assertContains(response, 'href="tel:07700900123"')

    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_dashboard_shows_detail_of_loss_popup_trigger_not_inline(self, mock_get_optimo, mock_get_handl):
        today = timezone.localdate()
        order_no = f"1001_{today.isoformat()}"
        mock_optimo = MagicMock()
        mock_optimo.list_orders_for_date.return_value = [
        ...
        mock_handl.get_job_details.return_value = {
            "1001": JobDetails(
                report_id="1001", make="Ford", model="Focus", year="2020", reg="AB20 CDE", vin="VIN1",
                service_type="Car", loss_type="LOST", supplied_service="", net_cost=100.0,
                detail_of_loss="Lost the only key on a dog walk near the park.",
            )
        }
        mock_get_handl.return_value = mock_handl
        ...
        response = self.client.get(reverse("locksmith_portal:dashboard"))
        job = response.context["jobs"][0]
        self.assertEqual(job["detail_of_loss"], "Lost the only key on a dog walk near the park.")
        self.assertContains(response, "Detail of loss")
        self.assertContains(response, "Lost the only key on a dog walk near the park.")
        self.assertContains(response, 'class="job-detail-btn"')
        self.assertContains(response, "<dialog")

    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_dashboard_hides_detail_of_loss_button_when_not_provided(self, mock_get_optimo, mock_get_handl):
        today = timezone.localdate()
        order_no = f"1001_{today.isoformat()}"
        mock_optimo = MagicMock()
        mock_optimo.list_orders_for_date.return_value = [
        ...
        response = self.client.get(reverse("locksmith_portal:dashboard"))
        self.assertNotContains(response, 'class="job-detail-btn"')
...
class JobDetailTests(TestCase):
    ...
    def test_post_disposes_within_stock_and_logs_portal_disposal(self, mock_get_optimo, mock_get_handl):
        ...
        self.assertFalse(disposal.client_supplied)

    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_client_supplied_known_sku_skips_stock_decrement(self, mock_get_optimo, mock_get_handl):
        self._mock_optimo(mock_get_optimo)
        mock_handl = MagicMock()
        mock_handl.list_current_stock.return_value = [
            CurrentStockLine(part_code="TK-100", part_name="Transponder key blank", qty=4),
        ...
        response = self.client.post(url, {
            "part_code": ["TK-100 — Transponder key blank"], "quantity": ["1"], "client_supplied": ["1"],
        })
    ...
    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_client_supplied_unknown_sku_still_accepted_and_flagged(self, mock_get_optimo, mock_get_handl):
        self._mock_optimo(mock_get_optimo)
        mock_handl = MagicMock()
        mock_handl.list_current_stock.return_value = []
        ...
        url = reverse("locksmith_portal:job_detail", args=[self.order_no])
        response = self.client.post(url, {
            "part_code": ["XYZ-999"], "quantity": ["1"], "client_supplied": ["1"],
        })
    ...
    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_client_supplied_can_exceed_van_stock_quantity(self, mock_get_optimo, mock_get_handl):
        self._mock_optimo(mock_get_optimo)
        mock_handl = MagicMock()
        mock_handl.list_current_stock.return_value = [
            CurrentStockLine(part_code="TK-100", part_name="Transponder key blank", qty=1),
        ]
        mock_get_handl.return_value = mock_handl

        url = reverse("locksmith_portal:job_detail", args=[self.order_no])
        response = self.client.post(url, {
            "part_code": ["TK-100 — Transponder key blank"], "quantity": ["5"], "client_supplied": ["1"],
        })

        self.assertRedirects(response, f"{url}?date={self.today.isoformat()}")
        disposal = PortalDisposal.objects.get()
        self.assertEqual(disposal.quantity, 5)

    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_missing_client_supplied_field_defaults_to_normal_disposal(self, mock_get_optimo, mock_get_handl):
        # A POST without the client_supplied field at all (e.g. an old
        # cached form) shouldn't silently drop every row — zip_longest
        # padding, not zip, is what makes this work.
        self._mock_optimo(mock_get_optimo)
        mock_handl = MagicMock()
        mock_handl.list_current_stock.return_value = [
            CurrentStockLine(part_code="TK-100", part_name="Transponder key blank", qty=4),
...
class FaultyPartReportTests(TestCase):
    ...
    path from a normal disposal: it still removes the part from van
    stock (via get_expected_stock + set_locksmith_stock_quantity), but
    never calls record_disposal, so it never shows up in Handl's
    Inventory_Disposals — apps.job_completion's parts-lookup "most
    likely part" suggestion reads real disposals from there, and a
    faulty part isn't one."""
    ...
    def setUp(self):
        ...
        JobVisit.objects.create(
            locksmith=self.locksmith, order_no=self.order_no, report_id="496390",
            stage=JobVisit.Stage.ARRIVED, arrived_at=timezone.now(),
            # Recorded already so _needs_access_method's own check never
            # comes into play here — these tests are about the faulty-
            # part path, not the access-method gate.
            access_method=JobVisit.AccessMethod.PICKED,
        )
        self.url = reverse("locksmith_portal:job_detail", args=[self.order_no])
    ...
    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_faulty_part_removes_stock_but_never_calls_record_disposal(
        self, mock_get_optimo, mock_get_handl
    ):
        self._mock_optimo(mock_get_optimo)
        mock_handl = MagicMock()
        ...
        mock_handl.get_expected_stock.return_value = {
            "TK-100": ExpectedStock(part_code="TK-100", expected_qty=4, unit_cost=5.0),
        }
        mock_handl.get_job_details.return_value = {
            "496390": JobDetails(
                report_id="496390", make="Ford", model="Focus", year="2020", reg="AB20 CDE",
                vin="VIN123", service_type="Car", loss_type="LOST", supplied_service="",
                net_cost=100.0, spare_key=False,
            )
        }
        mock_get_handl.return_value = mock_handl

        response = self.client.post(self.url, {
            "part_code": ["TK-100 — Transponder key blank"], "quantity": ["1"], "faulty": ["1"],
        })
        ...
        mock_handl.set_locksmith_stock_quantity.assert_called_once_with(
            ["885"], "TK-100", 3,  # 4 on hand - 1 faulty
            actioned_by_user_id=0, locksmith_display_name="Dean S",
        )
        note_text = mock_handl.add_report_note.call_args[0][1]
        self.assertIn("faulty", note_text)
        self.assertIn("removed from van stock", note_text)

        self.assertEqual(PortalDisposal.objects.count(), 0)
        report = FaultyPartReport.objects.get()
        self.assertEqual(report.part_code, "TK-100")
        self.assertEqual(report.quantity, 1)
        self.assertEqual(report.vin, "VIN123")
        self.assertEqual(report.reg, "AB20 CDE")
        self.assertEqual(report.make, "Ford")
        self.assertEqual(report.model_name, "Focus")
        self.assertEqual(report.year, "2020")
        self.assertIs(report.spare_key, False)
        self.assertTrue(report.handl_synced)

    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_faulty_checkbox_takes_priority_over_client_supplied(
        self, mock_get_optimo, mock_get_handl
    ):
        self._mock_optimo(mock_get_optimo)
        mock_handl = MagicMock()
        ...
        self.client.post(self.url, {
            ...
            "faulty": ["1"], "client_supplied": ["1"],
        })

        mock_handl.record_client_supplied_disposal.assert_not_called()
        self.assertEqual(FaultyPartReport.objects.count(), 1)
        self.assertEqual(PortalDisposal.objects.count(), 0)

    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_faulty_part_not_in_van_stock_is_rejected(self, mock_get_optimo, mock_get_handl):
        self._mock_optimo(mock_get_optimo)
        mock_handl = MagicMock()
        mock_handl.list_current_stock.return_value = []
        mock_handl.get_job_details.return_value = {}
        ...
        self.assertContains(response, "Couldn&#x27;t find")
        self.assertEqual(FaultyPartReport.objects.count(), 0)

    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_faulty_part_exceeding_stock_is_rejected(self, mock_get_optimo, mock_get_handl):
        self._mock_optimo(mock_get_optimo)
        mock_handl = MagicMock()
        mock_handl.list_current_stock.return_value = [
            CurrentStockLine(part_code="TK-100", part_name="Transponder key blank", qty=1),
        ...
        response = self.client.post(self.url, {
            "part_code": ["TK-100 — Transponder key blank"], "quantity": ["5"], "faulty": ["1"],
        }, follow=True)
        self.assertContains(response, "can&#x27;t report 5 faulty")
        self.assertEqual(FaultyPartReport.objects.count(), 0)
        mock_handl.set_locksmith_stock_quantity.assert_not_called()

    @patch("apps.locksmith_portal.views.get_handl_client")
    @patch("apps.locksmith_portal.views.get_optimo_client")
    def test_get_shows_previously_reported_faulty_parts(self, mock_get_optimo, mock_get_handl):
        self._mock_optimo(mock_get_optimo)
        mock_handl = MagicMock()
        mock_handl.list_current_stock.return_value = []
        mock_handl.get_job_details.return_value = {}
        ...
        self.assertContains(response, "Reported faulty")
        self.assertContains(response, "Transponder key blank")
...
class JobVisitWorkflowTests(TestCase):
    ...
    def test_gain_access_arrival_get_shows_named_photo_slots(self):
        self._set_loss_type("LOCKED IN PROPERTY")
        JobVisit.objects.create(
            locksmith=self.locksmith, order_no=self.order_no, report_id="496390",
            stage=JobVisit.Stage.ON_ROUTE, on_route_at=timezone.now(),
    ...
    def test_gain_access_arrival_missing_required_slot_is_rejected(self):
        self._set_loss_type("LOCKED IN PROPERTY")
        JobVisit.objects.create(
            locksmith=self.locksmith, order_no=self.order_no, report_id="496390",
            stage=JobVisit.Stage.ON_ROUTE, on_route_at=timezone.now(),
    ...
    def test_gain_access_arrival_success_advances_stage(self):
        self._set_loss_type("LOCKED IN PROPERTY")
        JobVisit.objects.create(
            locksmith=self.locksmith, order_no=self.order_no, report_id="496390",
            stage=JobVisit.Stage.ON_ROUTE, on_route_at=timezone.now(),
    ...
    def test_multiple_photos_in_one_slot_get_numbered_link_text(self):
        JobVisit.objects.create(
            locksmith=self.locksmith, order_no=self.order_no, report_id="496390",
            stage=JobVisit.Stage.ON_ROUTE, on_route_at=timezone.now(),
        )
    ...
    def test_gain_access_completion_key_in_hand_photo_is_optional(self):
        ...
        self.assertEqual(self._visit().photos.filter(kind=JobVisitPhoto.Kind.KEY_IN_HAND).count(), 0)

    # --- per-service completion flow: key-related (AKL / Spare Key / etc) ---
    ...
    def test_akl_arrival_get_shows_named_photo_slots(self):
        ...
        response = self.client.get(url)
        self.assertContains(response, "Front of the car")
        self.assertContains(response, "Damage")
        self.assertNotContains(response, "Door with the lock")
    ...
    def test_akl_arrival_missing_required_slot_is_rejected(self):
        ...
        url = reverse("locksmith_portal:job_arrived", args=[self.order_no])
        response = self.client.post(url, {})
        self.assertContains(response, "Add at least one photo: Front of the car")
        self.assertEqual(self._visit().stage, JobVisit.Stage.ON_ROUTE)
    ...
    def test_akl_get_shows_named_photo_slots(self):
        ...
        self.assertContains(response, "Blade turned in the door")
    ...
    def test_akl_success_uploads_all_named_slots(self):
        ...
        visit = self._visit()
        self.assertEqual(visit.photos.filter(kind=JobVisitPhoto.Kind.BLADE_IN_DOOR).count(), 1)
        self.assertEqual(visit.photos.filter(kind=JobVisitPhoto.Kind.BLADE_IN_IGNITION).count(), 1)
        self.assertEqual(visit.photos.filter(kind=JobVisitPhoto.Kind.CLIENT_KEY).count(), 0)

    def test_spare_key_arrival_get_shows_named_photo_slots(self):
    ...
    def test_spare_key_requires_client_key_photo(self):
        ...
        self.assertContains(response, "Add at least one photo: New key with the client&#x27;s key")
